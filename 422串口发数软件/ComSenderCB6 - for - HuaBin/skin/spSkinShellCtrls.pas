...
begin
  ...
  then
    begin
      ToolPanel := TspSkinPanel.Create(Self);
      with ToolPanel do
      begin
        Parent := Self;
        Align := alTop;
...
begin
  FDlgFrm := TspOpenDlgForm.CreateEx(Application, FSaveMode, CtrlSkinData, FToolButtonsTransparent);
...
var
  ...
  Path: String;
begin
  FDlgFrm := TspOpenSkinDlgForm.CreateEx(Application, CtrlSkinData, FToolButtonsTransparent);
  with FDlgFrm do
  try
    Caption := Self.Title;
    DSF.BorderIcons := [];
...
var
  ...
  Ext1, Ext2, Path: String;
begin
  FDlgFrm := TspOpenPreviewDlgForm.CreateEx(Application, FSaveMode, CtrlSkinData, FToolButtonsTransparent);
  FDlgFrm.OnFolderChange := FOnFolderChange;
  FDlgFrm.CheckFileExists := FCheckFileExists;
...
var
  ...
  Ext1, Ext2, Path: String;
begin
  FDlgFrm := TspOpenSoundDlgForm.CreateEx(Application, FSaveMode, CtrlSkinData, FToolButtonsTransparent);
  FDlgFrm.OnFolderChange := FOnFolderChange;
  FDlgFrm.CheckFileExists := FCheckFileExists;
  FDlgFrm.OverwritePromt := OverwritePromt;
  with FDlgFrm do
