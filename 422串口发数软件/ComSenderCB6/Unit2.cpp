...
//---------------------------------------------------------------------------
void __fastcall TForm2::RecData2(unsigned char c)
{
    char ctmp[260] = {0};
    unsigned int i = c;

    sprintf(ctmp, "%XH", i);
    spSkinListView1->Items->Item[6]->SubItems->Strings[0] = ctmp;
    
    switch(Form1->m_nWorkMode)
    {
        case 17:
        {
            double tmp = ((double)c / 255.0)*2.5 + 2.5;
            sprintf(ctmp, "%.2fV", tmp);
